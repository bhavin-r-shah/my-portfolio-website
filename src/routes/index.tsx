...
export const Route = createFileRoute("/")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "React, Go, cloud-native systems, design systems, and accessibility. 15 years bridging product and engineering." },
...
const skillGroups = [
  { icon: Code2, label: "Frontend", items: ["React", "TypeScript", "Stencil.js", "Tailwind CSS", "Cypress", "Jest"] },
  { icon: Cpu, label: "Backend", items: ["Go (Golang)", "Java", "REST", "gRPC", "Kafka", "PostgreSQL", "Redis", "MongoDB"] },
  { icon: Layers, label: "Platform", items: ["GitHub Actions", "Jenkins", "Docker", "Git", "Unix"] },
  { icon: Sparkles, label: "AI-Assisted Dev", items: ["GitHub Copilot", "VS Code AI", "ChatGPT workflows"] },
  { icon: Gauge, label: "Quality & UX", items: ["WCAG 2.2 AA", "Design Systems", "Figma", "Web Vitals"] },
  { icon: Leaf, label: "Sustainability", items: ["Tree-shaking", "CDN-first", "Lazy loading", "Dark theme"] },
];
            ...
            >
              ...
              <div className="flex flex-1 flex-col p-6">
                ...
                <span className="mt-6 inline-flex items-center gap-1 text-sm font-medium text-primary">
                  Case study <ArrowUpRight className="h-4 w-4 transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />
                </span>
              </div>
            </Link>
          ))}
        ...
        <div className="container-page py-20">
          <div className="flex items-end justify-between gap-6">
            <div>
              ...
              <h2 className="display-serif mt-2 text-4xl sm:text-5xl">A working stack, not a wishlist.</h2>
            ...
            {skillGroups.map(({ icon: Icon, label, items }) => (
              <div key={label} className="card-surface p-6">
                ...
                <ul className="mt-4 flex flex-wrap gap-1.5">
                  {items.map((i) => (
                    <li key={i} className="rounded-full border border-border px-2.5 py-1 font-mono text-[11px] text-muted-foreground">
                      {i}
                    </li>
                  ))}
                </ul>
        ...
        <div className="grid gap-10 md:grid-cols-[1fr_1.4fr] md:items-center">
          <div>
            ...
            <p className="mt-4 text-sm text-muted-foreground">
              GitHub is the working notebook — components, patterns, and small
              tools that back up the case studies. Open source contributions
              and side projects land here as I ship them.
            </p>
              ...
              {Array.from({ length: 371 }).map((_, i) => {
                ...
                return (
                  <span
                    ...
                    style={{ background: `color-mix(in oklab, var(--primary) ${alpha * 100}%, var(--surface))` }}
                  />
                );
              })}
            </div>
            <div className="mt-4 flex items-center justify-between text-[11px] text-muted-foreground">
              ...
              <div className="flex items-center gap-1">
                ...
                {[0.1, 0.25, 0.45, 0.7].map((a) => (
                  <span key={a} className="h-2.5 w-2.5 rounded-[2px]" style={{ background: `color-mix(in oklab, var(--primary) ${a * 100}%, var(--surface))` }} />
                ))}
                <span>More</span>
              </div>
            </div>
      ...
      <section className="container-page py-20">
        <div className="grid gap-10 md:grid-cols-[1fr_1.3fr] md:items-center">
          ...
          <div>
            ...
            <h2 className="display-serif mt-2 text-4xl sm:text-5xl">Product-minded. Engineering-native.</h2>
            <p className="mt-4 text-sm leading-relaxed text-muted-foreground">
              I trained as a computer scientist (M.S., Northeastern) and spent
              the last decade+ shipping software at HPE, Nimble Storage,
              Intuit, IBM, and Accenture. I've owned features end-to-end — from
              customer discovery and system design through UI, APIs, storage,
              CI/CD, and iteration — and I lead teams as an Agile Scrum Master.
            </p>
            <div className="mt-6 flex gap-3">
              <CtaLink href="/about" variant="secondary">More about me <ArrowRight className="h-4 w-4" /></CtaLink>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="container-page pb-24">
        ...
        >
          ...
          <p className="mt-4 max-w-xl text-muted-foreground">
            View my résumé, explore my GitHub, or contact me directly. I reply
